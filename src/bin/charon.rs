...
//!
//! ```text
//! charon separate <input> [-o DIR] [--model FILE] [--format F] [--shifts N] [--ep cpu|coreml|auto] [--no-server]
...
use charon_audio::{BitDepth, ExecutionProvider, Separator, SeparatorConfig, StemFormat};
...
/// Model contract from the file name: `*split*` is the split-transform
/// export, anything else the in-graph export.
fn config_for(model: &Path, ep: Option<ExecutionProvider>) -> SeparatorConfig {
    let name = model.file_name().and_then(|n| n.to_str()).unwrap_or("");
    let mut config = if name.contains("split") {
...
fn serve(o: &Options) -> Result<(), String> {
    ...
    let t = Instant::now();
    let separator = Separator::new(config_for(&model, o.ep)).map_err(|e| e.to_string())?;
    eprintln!(
...
fn separate(o: &Options) -> Result<(), String> {
    ...
    let mut config = config_for(&model, o.ep);
...
fn main() {
    ...
    let usage = "usage: charon separate <input> [-o DIR] [--model FILE] [--format wav16|wav24|wav32|flac16|flac24] [--shifts N] [--ep cpu|coreml|auto] [--socket PATH] [--no-server]\n       charon serve --model FILE [--ep ...] [--socket PATH]\n       charon stop [--socket PATH]\n       charon ping [--socket PATH]";
